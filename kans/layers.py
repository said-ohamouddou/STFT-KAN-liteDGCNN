...
class KANLayer(nn.Module):
    def __init__(self, input_features, output_features, grid_size=1, spline_order=0, base_activation=nn.GELU,
                 grid_range=[-1, 1]):
...
class KALNLayer(nn.Module):  # Kolmogorov Arnold Legendre Network (KAL-Net)
    def __init__(self, input_features, output_features, degree=0, base_activation=nn.SiLU):
...
class GRAMLayer(nn.Module):
    def __init__(self, in_channels, out_channels, degree=0, act=nn.SiLU):
...
class JacobiKANLayer(nn.Module):
    def __init__(self, input_dim, output_dim, degree=0, a=1/2, b=1/2, act=nn.SiLU):
