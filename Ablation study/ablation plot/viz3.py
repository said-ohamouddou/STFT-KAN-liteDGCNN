...
def load_and_process_data(csv_file):
    ...
    # Convert Parameter_Value to numeric, handling any string values
    try:
        df['Parameter_Value'] = pd.to_numeric(df['Parameter_Value'], errors='coerce')
        print(f"Converted Parameter_Value to numeric. {df['Parameter_Value'].isna().sum()} non-numeric values found.")
    except Exception as e:
        print(f"Warning: Could not convert Parameter_Value to numeric: {e}")
...
def get_parameter_label(param_type, layer_num):
    """Get properly formatted parameter label with descriptive names and layer names"""
    # Map parameter types to descriptive names
    param_names = {
        'g': 'Grid Size',
        'w': 'Window Size', 
        's': 'Stride',
    ...
    param_name = param_names.get(param_type, param_type.upper())
    return f'{param_name} ({layer_name})'
...
def create_parameter_performance_plots(df, font_size=10):
    """Create combined sensitivity plots for all numeric parameters"""
    ...
    available_param_types = [pt for pt in numeric_param_types if pt in df_processed['ParamType'].unique()]
    
    if not available_param_types:
        print("Warning: No numeric parameters (g, w, s) found")
        return
    
    # Create combined subplot figure
    fig, axes = plt.subplots(1, len(available_param_types), figsize=(6*len(available_param_types), 4))
    
    # Handle case where there's only one subplot
    if len(available_param_types) == 1:
        axes = [axes]
    ...
    for idx, param_type in enumerate(available_param_types):
        ax = axes[idx]
        param_data = df_processed[df_processed['ParamType'] == param_type].copy()
        ...
        ax.set_xlabel('Parameter Value', fontsize=font_size)
        if idx == 0:  # Only leftmost plot gets y-label
            ax.set_ylabel('Accuracy', fontsize=font_size)
        ax.grid(True, alpha=0.3, linewidth=0.5)
        ax.set_title(f'{param_names[param_type]} Sensitivity', 
                    fontsize=font_size+1, fontweight='bold')
    
    # Create shared horizontal legend below the plots
    handles, labels = axes[0].get_legend_handles_labels()
    fig.legend(handles, labels, loc='lower center', bbox_to_anchor=(0.5, -0.15), 
              ncol=len(labels), fontsize=font_size, frameon=False)
    
    plt.tight_layout()
    plt.subplots_adjust(bottom=0.2)  # Make room for legend
    plt.savefig('parameter_sensitivity_combined.pdf', format='pdf')
    plt.show()
...
def create_numeric_parameter_summary(param_data, param_type, font_size):
    ...
    # Special legend positioning for Grid Size plot
    if param_type == 'g':
        # Move legend left by 1cm (~0.39 inches) and down by 0.5cm (~0.20 inches)
        ax.legend(bbox_to_anchor=(-0.39, -0.20), loc='upper right', fontsize=font_size-2)
    else:
...
def create_best_performance_heatmap(df, font_size=10):
    ...
    try:
        ...
        param_order = ['Grid Size', 'Window Size', 'Stride', 'Smooth Init', 'Window Type']
...
def create_parameter_impact_analysis(df, font_size=10):
    ...
    param_names = {'g': 'Grid Size', 'w': 'Window Size', 's': 'Stride'}
    ...
    for i, param_type in enumerate(['g', 'w', 's']):
        ...
        ax_top.set_title(f'{param_names[param_type]}: Model Size Impact', 
        ...
        ax_bottom.set_title(f'{param_names[param_type]}: Training Time Impact', 
                           fontsize=font_size, fontweight='bold')
    ...
    for param_type in ['g', 'w', 's']:
        ...
        print(f"\n{param_names[param_type]} Analysis:")
...
def main(font_size=10):
    ...
    print("\n4. Creating parameter impact analysis...")
    ...
    print("Files saved:")
    print("- parameter_sensitivity_combined.pdf")
